...
const TextAnalyzer = ({ onDataExtracted }) => {
  ...
  const processTextWithOpenAI = async () => {
    if (!conversationText.trim()) {
      Alert.alert(
        "Texto vacío",
        "Por favor, ingrese la conversación para analizar"
      );
      return;
    }
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "Extrae la siguiente información de la transcripción: nombre completo, número de teléfono, código postal, nombre del negocio y tipo de negocio. Devuelve un objeto JSON con estos campos.",
          },
          {
            role: "user",
      ...
      if (!extractionResponse.ok) {
        ...
        throw new Error(
          `Error en la API de OpenAI: ${extractionResponse.status} ${errorText}`
        );
      }
      ...
      if (
        !extractionData ||
        !extractionData.choices ||
        extractionData.choices.length === 0
      ) {
      ...
      Alert.alert(
        ...
        "Se han completado los campos con la información de la conversación."
      );
      ...
      Alert.alert(
        "Error",
        `No se pudo procesar la transcripción: ${error.message}`
      );
      setIsProcessing(false);
    }
  };
  ...
  return (
    <View className="mb-5">
      <View className="mb-3">
        <Text className="text-sm text-text-soft mb-1">
          Ingrese la conversación con el microempresario
        </Text>
        <TextInput
