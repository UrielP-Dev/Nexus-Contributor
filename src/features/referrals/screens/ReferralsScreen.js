...
import { View, Text, FlatList, ActivityIndicator, Alert, ScrollView, TouchableOpacity, Image } from 'react-native';
...
const mockMicroempresarios = [
  { id: '1', name: 'María', last_name: 'González', email: 'maria.gonzalez@example.com', phone: '5551234567', business_name: 'Artesanías María', business_type: 'Artesanías', registration_date: '2023-10-15' },
  { id: '2', name: 'Juan', last_name: 'Pérez', email: 'juan.perez@example.com', phone: '5552345678', business_name: 'Abarrotes Don Juan', business_type: 'Tienda', registration_date: '2023-10-18' },
  { id: '3', name: 'Ana', last_name: 'Rodríguez', email: 'ana.rodriguez@example.com', phone: '5553456789', business_name: 'Cosméticos Ana', business_type: 'Belleza', registration_date: '2023-10-20' },
  { id: '4', name: 'Carlos', last_name: 'López', email: 'carlos.lopez@example.com', phone: '5554567890', business_name: 'Taller Mecánico López', business_type: 'Servicios', registration_date: '2023-10-25' },
  { id: '5', name: 'Patricia', last_name: 'Martínez', email: 'patricia.martinez@example.com', phone: '5555678901', business_name: 'Papelería Patty', business_type: 'Papelería', registration_date: '2023-11-02' },
  { id: '6', name: 'Miguel', last_name: 'Sánchez', email: 'miguel.sanchez@example.com', phone: '5556789012', business_name: 'Taquería Don Miguel', business_type: 'Alimentos', registration_date: '2023-11-05' },
  { id: '7', name: 'Laura', last_name: 'Torres', email: 'laura.torres@example.com', phone: '5557890123', business_name: 'Boutique Elegante', business_type: 'Ropa', registration_date: '2023-11-10' },
  { id: '8', name: 'Roberto', last_name: 'Flores', email: 'roberto.flores@example.com', phone: '5558901234', business_name: 'Ferretería Flores', business_type: 'Ferretería', registration_date: '2023-11-15' },
  { id: '9', name: 'Sofía', last_name: 'Ramírez', email: 'sofia.ramirez@example.com', phone: '5559012345', business_name: 'Pastelería Sofía', business_type: 'Panadería', registration_date: '2023-11-20' },
  { id: '10', name: 'Daniel', last_name: 'Mendoza', email: 'daniel.mendoza@example.com', phone: '5550123456', business_name: 'Electrónica Mendoza', business_type: 'Electrónica', registration_date: '2023-11-25' },
  { id: '11', name: 'Carmen', last_name: 'Herrera', email: 'carmen.herrera@example.com', phone: '5551234567', business_name: 'Florería Carmen', business_type: 'Florería', registration_date: '2023-12-01' },
  { id: '12', name: 'Javier', last_name: 'Díaz', email: 'javier.diaz@example.com', phone: '5552345678', business_name: 'Carpintería Díaz', business_type: 'Carpintería', registration_date: '2023-12-05' },
  { id: '13', name: 'Elena', last_name: 'Castro', email: 'elena.castro@example.com', phone: '5553456789', business_name: 'Estética Elena', business_type: 'Belleza', registration_date: '2023-12-10' },
  { id: '14', name: 'Fernando', last_name: 'Ortiz', email: 'fernando.ortiz@example.com', phone: '5554567890', business_name: 'Zapatería Ortiz', business_type: 'Calzado', registration_date: '2023-12-15' },
  { id: '15', name: 'Gabriela', last_name: 'Vargas', email: 'gabriela.vargas@example.com', phone: '5555678901', business_name: 'Cafetería Gaby', business_type: 'Cafetería', registration_date: '2023-12-20' }
];

// Datos simulados de webinars disponibles
const mockWebinars = [
  { id: '1', title: 'Fundamentos del Negocio Digital', date: '2023-10-10', duration: '60 min' },
  { id: '2', title: 'Marketing para Microempresas', date: '2023-10-20', duration: '45 min' },
  { id: '3', title: 'Gestión Financiera Básica', date: '2023-11-05', duration: '90 min' },
  { id: '4', title: 'Atención al Cliente', date: '2023-11-15', duration: '60 min' },
  { id: '5', title: 'Estrategias de Crecimiento', date: '2023-12-01', duration: '75 min' }
];
...
const generateMockData = (id) => {
  ...
  const daysAgo = isHighPerformer ? Math.floor(Math.random() * 3) : 
                  isLowPerformer ? Math.floor(Math.random() * 14) + 7 : 
                  Math.floor(Math.random() * 7);
  const lastActivity = new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
...
const ProgressBar = ({ progress, label, emoji, color = "#006FB9" }) => (
  <View className="mb-4">
    <View className="flex-row justify-between mb-1">
      <Text className="text-text-soft text-sm">{emoji} {label}</Text>
      <Text className="text-text-soft text-sm font-medium">{Math.round(progress * 100)}%</Text>
    </View>
...
const StatCard = ({ title, value, total, emoji }) => (
  <View className="bg-white rounded-xl p-4 shadow-sm flex-1 mx-1">
    ...
    <Text className="text-center text-primary text-xl font-bold">{value}/{total}</Text>
  </View>
);

// Componente para incentivos con niveles y candados
const IncentiveLevel = ({ level, title, description, icon, isUnlocked, isActive }) => (
  <View className={`bg-white rounded-xl p-4 mb-3 shadow-sm ${isActive ? 'border-2 border-primary' : ''}`}>
    <View className="flex-row items-center">
      <View className={`w-12 h-12 rounded-full justify-center items-center mr-3 ${isUnlocked ? 'bg-primary-light' : 'bg-gray-200'}`}>
      ...
      <View className="flex-1">
        <View className="flex-row items-center">
          ...
          {!isUnlocked && (
            <Ionicons name="lock-closed" size={16} color="#888" style={{ marginLeft: 6 }} />
          )}
          {isUnlocked && (
...
const MicroempresarioCard = ({ referral, progressData, onPress, isSelected }) => (
  <TouchableOpacity 
    onPress={onPress}
    className={`bg-white rounded-xl p-4 mb-3 shadow-sm ${isSelected ? 'border-2 border-primary' : ''}`}
  >
    <View className="flex-row justify-between items-center mb-2">
      <View className="flex-row items-center">
        <Text className="text-xl mr-2">👤</Text>
        <View>
          <Text className="font-bold">{referral.name} {referral.last_name}</Text>
    ...
    <View className="mb-2">
      <View className="flex-row justify-between mb-1">
        ...
        <Text className="text-text-soft text-xs">{Math.round(progressData.progress * 100)}%</Text>
      </View>
      <View className="h-2 bg-gray-200 rounded-full">
        <View 
          ...
          style={{ 
            ...
            backgroundColor: progressData.progress > 0.7 ? '#4CAF50' : progressData.progress > 0.3 ? '#FF9800' : '#F44336' 
          }}
        />
      </View>
    </View>
    ...
    <View className="flex-row justify-between items-center">
      <Text className="text-text-soft text-xs">
        <Ionicons name="time-outline" size={12} /> Última actividad: {progressData.lastActivity}
      </Text>
    ...
    {progressData.keysCollected > progressData.webinarsCompleted * 7 && (
      <View className="bg-yellow-50 p-3 rounded-lg mt-3">
        ...
        <Text className="text-yellow-800 mt-1">
          Este microempresario ha obtenido {progressData.keysCollected - (progressData.webinarsCompleted * 7)} llaves adicionales por actividades complementarias.
        </Text>
      </View>
    )}
    ...
    <View className="bg-blue-50 p-3 rounded-lg mt-4">
      ...
      <Text className="text-text-soft mt-1">
        {progressData.webinarsCompleted === progressData.totalWebinars 
          ...
          : `⏳ Le faltan ${progressData.totalWebinars - progressData.webinarsCompleted} webinars por completar.`}
      </Text>
      <Text className="text-text-soft mt-1">
        {progressData.keysCollected === progressData.totalKeys 
          ? '✅ Ha recolectado todas las llaves.' 
          : `⏳ Le faltan ${progressData.totalKeys - progressData.keysCollected} llaves por recolectar.`}
      </Text>
      <Text className="text-text-soft mt-1">
        📊 Se encuentra en el percentil {progressData.percentileRanking} de todos los microempresarios.
      </Text>
    </View>
...
const ReferralsScreen = ({ navigation }) => {
  ...
  const getSortedReferrals = () => {
    ...
    return [...referrals].sort((a, b) => {
      ...
      switch (sortOption) {
        ...
        case 'recent':
          return new Date(progressB.lastActivity) - new Date(progressA.lastActivity);
        default:
          return 0;
      }
    });
  ...
  const calculateStats = () => {
    if (Object.keys(referralProgressData).length === 0) return {
      totalWebinars: 0,
    ...
    const stats = Object.values(referralProgressData).reduce((acc, data) => {
      return {
        totalWebinars: acc.totalWebinars + data.totalWebinars,
        totalWebinarsCompleted: acc.totalWebinarsCompleted + data.webinarsCompleted,
        ...
        lowestProgress: acc.lowestProgress === 0 ? data.progress : Math.min(acc.lowestProgress, data.progress),
      };
    }, {
      totalWebinars: 0,
      totalWebinarsCompleted: 0,
          ...
          <View className="px-4 py-6">
            ...
            <View className="bg-white rounded-xl p-4 mb-6 shadow-sm">
              <View className="flex-row justify-between items-center mb-4">
                <View>
                  ...
                  <Text className="text-text-soft">Nivel actual: {userProgress.nivel}</Text>
                </View>
                <View className="bg-primary-light px-3 py-1 rounded-full">
                  <Text className="text-primary font-medium">{userProgress.totalRegistros} Registros</Text>
                </View>
              </View>
              ...
              <View className="mb-4">
                <View className="flex-row justify-between mb-1">
                  <Text className="text-text-soft text-sm">🎯 Meta semanal ({userProgress.registrosSemana}/{userProgress.metaSemanal})</Text>
                  <Text className="text-text-soft text-sm font-medium">
                    {Math.round((userProgress.registrosSemana / userProgress.metaSemanal) * 100)}%
                  </Text>
                </View>
                <View className="h-2.5 bg-gray-200 rounded-full">
                  <View 
                    className="h-2.5 rounded-full bg-green-500" 
                    style={{ width: `${(userProgress.registrosSemana / userProgress.metaSemanal) * 100}%` }}
                  />
                </View>
              </View>
              ...
              <View className="flex-row mt-2">
                <View className="bg-blue-50 rounded-lg p-3 flex-1 mr-2">
                  <Text className="text-center text-sm text-text-soft">Esta semana</Text>
                  <Text className="text-center text-primary text-xl font-bold">{userProgress.registrosSemana}</Text>
                </View>
                <View className="bg-purple-50 rounded-lg p-3 flex-1">
                  <Text className="text-center text-sm text-text-soft">Total</Text>
                  <Text className="text-center text-purple-600 text-xl font-bold">{userProgress.totalRegistros}</Text>
                </View>
              </View>
            </View>
            
            {/* Niveles de Incentivos */}
            <Text className="text-lg font-bold mb-3">Niveles de Recompensa 🏆</Text>
            <View className="mb-6">
              <IncentiveLevel 
                level={1}
                title="Nivel Básico" 
            ...
            <View className="mb-6">
              <View className="flex-row justify-between items-center mb-3">
                <Text className="text-lg font-bold">Progreso de Microempresarios 📊</Text>
                <View className="bg-gray-100 px-3 py-1 rounded-full">
                  <Text className="text-xs font-medium">{userProgress.totalRegistros} registros</Text>
                </View>
              </View>
              ...
              <View className="bg-white rounded-xl p-4 shadow-sm mb-4">
                ...
                <ProgressBar 
                  progress={stats.totalWebinarsCompleted / (stats.totalWebinars || 1)} 
                ...
                <View className="bg-blue-50 p-3 rounded-lg mt-3">
                  <Text className="text-primary font-medium">
                    💡 Tip: Contacta a los microempresarios con menor progreso para ayudarles a avanzar.
                  </Text>
                </View>
              </View>
              ...
              <View className="flex-row justify-between bg-white rounded-xl p-2 mb-3 shadow-sm">
                <TouchableOpacity 
                  ...
                  className={`flex-1 p-2 rounded-lg ${sortOption === 'keys' ? 'bg-primary-light' : ''}`}
                >
                  <Text className={`text-center ${sortOption === 'keys' ? 'text-primary font-medium' : 'text-text-soft'}`}>
                    🔑 Llaves
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  onPress={() => setSortOption('progress')}
                  className={`flex-1 p-2 rounded-lg ${sortOption === 'progress' ? 'bg-primary-light' : ''}`}
                >
                  <Text className={`text-center ${sortOption === 'progress' ? 'text-primary font-medium' : 'text-text-soft'}`}>
                    📈 Progreso
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  onPress={() => setSortOption('recent')}
                  className={`flex-1 p-2 rounded-lg ${sortOption === 'recent' ? 'bg-primary-light' : ''}`}
                >
                  <Text className={`text-center ${sortOption === 'recent' ? 'text-primary font-medium' : 'text-text-soft'}`}>
                    🕒 Reciente
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            <Text className="text-lg font-bold mb-3">Microempresarios Registrados</Text>
            <FlatList
